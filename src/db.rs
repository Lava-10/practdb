...
pub(crate) fn practDb_meta_schema() -> Schema {
    Schema::from(&[
...
impl<F: Seek + Read + Write + FileOps> Database<F> {
    ...
    ///
    /// Right now the [`practDb_META`] table doesn't use any indexes, so this
    ...
    fn load_table_metadata(&mut self, table: &str) -> Result<TableMetadata, DbError> {
        if table == practDb_META {
            let mut schema = practDb_meta_schema();
            schema.prepend_row_id();

            return Ok(TableMetadata {
                root: practDb_META_ROOT,
                name: String::from(table),
                row_id: self.load_next_row_id(practDb_META_ROOT)?,
                schema,
                indexes: vec![],
            });
        }
        ...
        let (schema, mut results) = self.prepare(&format!(
            "SELECT root, sql FROM {practDb_META} where table_name = '{table}';"
        ))?;

        let corrupted_error = || {
            DbError::Corrupted(format!(
                "{practDb_META} table is corrupted or contains wrong/unexpected data"
            ))
        };
    ...
    fn index_metadata(&mut self, index_name: &str) -> Result<IndexMetadata, DbError> {
        let query = self.exec(&format!(
            "SELECT table_name FROM {practDb_META} where name = '{index_name}' AND type = 'index';"
        ))?;
...
mod tests {
    ...
    use crate::{
        db::{practDb_meta_schema, QuerySet, Schema, SqlError, TypeError},
        paging::{
    ...
    fn create_table_auto_index() -> Result<(), DbError> {
        ...
        let query = db.exec("SELECT * FROM practDb_meta;")?;

        assert_eq!(
            query,
            QuerySet::new(practDb_meta_schema(), vec![vec![
    ...
    fn create_table_with_forced_pk_index() -> Result<(), DbError> {
        ...
        assert_eq!(
            query,
            QuerySet::new(practDb_meta_schema(), vec![
                vec![
                    Value::String("table".into()),
                    Value::String("users".into()),
                    Value::Number(1),
    ...
    fn create_many_tables_with_forced_pk_index() -> Result<(), DbError> {
        ...
        let query = db.exec("SELECT * FROM practDb_meta ORDER BY table_name, name;")?;
    ...
    fn drop_table() -> Result<(), DbError> {
        ...
        let practDb_meta_query = db.exec("SELECT * FROM practDb_meta;")?;
