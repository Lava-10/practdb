        ...
        while let Some(tuple) = self.source.try_next()? {
            if !self.mem_buf.can_fit(&tuple) {
                if self.file.is_none() {
                    let (file_path, file) = tmp_file(&self.work_dir, "practDb.query")?;
...
impl<F: Seek + Read + Write + FileOps> Sort<F> {
    ...
    fn sort(&mut self) -> Result<(), DbError> {
        ...
        let (input_file_path, input_file) = tmp_file::<F>(&self.work_dir, "practDb.sort.input")?;
        self.input_file = Some(input_file);
        self.input_file_path = input_file_path;

        let (output_file_path, output_file) = tmp_file::<F>(&self.work_dir, "practDb.sort.output")?;
...
impl<F: Seek + Read + Write + FileOps> PageRunsFifo<F> {
    ...
    fn push_back(&mut self, run: usize) -> io::Result<()> {
        ...
        if self.file.is_none() {
            let (path, file) = tmp_file::<F>(&self.work_dir, "practDb.sort.runs")?;
            self.file = Some(file);
            self.file_path = path;
        }
...
fn tmp_file<F: FileOps>(work_dir: &Path, extension: &str) -> io::Result<(PathBuf, F)> {
    ...
    let path = work_dir.join(format!("practDb.tmp/{file_name:x}.{extension}"));
