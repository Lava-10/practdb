...
use crate::{
    db::{
        has_btree_key, practDb_meta_schema, Database, DatabaseContext, DbError, IndexMetadata, RowId,
        Schema, SqlError, practDb_META, practDb_META_ROOT,
    },
...
pub(crate) fn exec<F: Seek + Read + Write + FileOps>(
    ...
    match statement {
        Statement::Create(Create::Table { name, columns }) => {
            ...
            insert_into_practDb_meta(db, vec![
        ...
        Statement::Drop(Drop::Table(name)) => {
            let comparator = db.table_metadata(practDb_META)?.comparator()?;

            let mut plan = collect_from_practDb_meta_where(db, &format!("table_name = '{name}'"))?;

            let schema = plan.schema().ok_or(DbError::Corrupted(format!(
                "could not obtain schema of {practDb_META} table"
            )))?;
            ...
            while let Some(tuple) = plan.try_next()? {
                ...
                BTree::new(&mut db.pager.borrow_mut(), practDb_META_ROOT, comparator).remove(
...
/// Inserts data into the [`practDb_META`] table.
///
...
/// #[rustfmt::skip]
/// db.exec(r#"
///     INSERT INTO practDb_meta (type, name, root, table_name, sql)
...
fn insert_into_practDb_meta<F: Seek + Read + Write + FileOps>(
    db: &mut Database<F>,
    ...
    values.insert(
        0,
        Value::Number(db.table_metadata(practDb_META)?.next_row_id().into()),
    );
...
fn collect_from_practDb_meta_where<F: Seek + Read + Write + FileOps>(
    db: &mut Database<F>,
    filter: &str,
    ...
    let table = db.table_metadata(practDb_META)?;
    ...
    Ok(Plan::Collect(Collect::from(CollectConfig {
        ...
        source: Box::new(Plan::Filter(Filter {
            ...
            source: Box::new(Plan::SeqScan(SeqScan {
                ...
                pager: Rc::clone(&db.pager),
                cursor: Cursor::new(practDb_META_ROOT, 0),
            })),
        })),
    })))
}
