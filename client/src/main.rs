//! Simple CLI client for practDb.
//!
//! See [`practDb::tcp::proto`] for a detailed description of the network protocol.
...
use practDb::{tcp::proto::Response, Value};
use rustyline::{error::ReadlineError, DefaultEditor};

const EXIT_CMD: &str = "quit";
const PROMPT: &str = "practDb> ";
...
fn main() -> rustyline::Result<()> {
    ...
    if rl.load_history("history.practDb").is_err() {
        println!("No previous history.");
    }

    let mut stream = TcpStream::connect(("127.0.0.1", port))?;
    println!("Connected to {}.", stream.peer_addr()?);
    println!("Welcome to the practDb shell. Type SQL statements below or '{EXIT_CMD}' to exit the program.\n");
    ...
    loop {
        ...
        while let Some(pos) = terminator_positions.pop_front() {
            ...
            match practDb::tcp::proto::deserialize(&payload) {
    ...
    rl.save_history("history.practDb")?;
    Ok(())
}
...
fn ascii_table(query: &practDb::QuerySet) -> String {
